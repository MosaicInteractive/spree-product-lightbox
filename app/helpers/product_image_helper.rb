module ProductImageHelper
  def link_to_main_product_image(product)
    ...
    link_to product_image(product, :alt => product.name), large_image_url, :title => product.name
  end
  
  def link_to_image(image, title='')
    link_to image_tag(image.url(:mini), :alt => product.name), image.url(:large), :title => title
  end
end
